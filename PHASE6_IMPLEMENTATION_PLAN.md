...
asset, generic finite structured data, the paired `ItemsData` editor, date/time
editing, table-data editing, composite Inspector rows, and column-settings are
implemented. The remaining 6.9 work is final integration and deferral records.
The runtime catalog continues to use the Inspector's readable, disabled fallback
for entries whose specialized editor remains deliberately deferred.

Complete the remaining work in this order:

1. Record the remaining deferrals, source round-trip coverage, focus behavior,
   and R2024a full-suite result in 6.9.10.
...
- [x] Add a separate versioned `inspector-rows.json` development/runtime
...
- [x] Load allowlisted typed `InspectorRowDefinition` templates into the
...
- [x] Generalize Inspector row construction, synchronization, transient drafts,
...
- [x] Replace `uitable`'s three duplicate table-data launch rows with one
...
- [x] Add one inline `Font Style` row for `FontWeight` and `FontAngle`, using
  ...
  reviewed.
- [x] Record high-confidence follow-up candidates without implementing them.
  Axes-family and full-font composites remain separate vertical slices after
  their row name, controls, error routing, and commit semantics are reviewed.
- [x] Verify strict catalog failures, deterministic row projection, unchanged
...
**Status:** Complete. Follow-up composite candidates remain deferred until they
have a natural user-facing name and a reviewed editor contract.
...
- [x] Use one metadata-driven modal dialog for all five properties. The single
...
- [x] Provide per-column controls plus audited all-column actions: all on/off for
...
- [x] Keep changes in a local draft and apply all changed column properties,
...
- [x] Accept the documented shorter and longer per-column forms. Missing
...
- [x] Disable per-column mutation when effective Data is unsupported or
...
**Status:** Complete. The dialog uses the shared atomic mutation path; the
remaining whole-Phase source round-trip and full-suite record belongs to 6.9.10.
